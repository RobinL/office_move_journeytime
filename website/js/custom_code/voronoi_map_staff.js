voronoiMap = function(map, url) {
    ...
    var update_hover_panel = function() {
        ...
        var h_dict = {
            ...
            new_journey_time: point.new_journey_time,
            original_journey_time: point.original_journey_time,
            change_in_journey_time: point.change_in_journey_time,
            lat: point.lat,
            lng: point.lng
        };
    ...
    var getListOfMetrics = function() {
        ...
        var keys = _.filter(_.keys(points[0]), function(d) {
            return !_.contains(["postcode", "lng", "lat"], d)
        })
    ...
    d3.select("#opacity_option").on("change", function(d) {
            drawWithLoading()
        })
    ...
    var drawMetricSelection = function() {
        ...
        $('#metricOptions').val("change_in_journey_time")
    ...
    var draw = function() {
        ...
        vor_points.append("circle")
            ...
            .style('fill', function(d) {
                return changeTravelTimeScale(d.change_in_journey_time)
            })
            .attr("r", 3);
    ...
    function makeChangeTravelTimeScale(points) {

        changeTravelTimeScale = d3.scale.linear()
                .domain([-20,0,20])
                .range(colourOptions["Red (high) to green (low)2"]);
    }
    ...
    map.on('ready', function() {
        ...
        d3.json("uk.json", function(uk_data) {
            ...
            d3.csv("data/all_staff_journeytime_final.csv", function(data) {
                ...
                drawColourSelection();
                setColourScale();
                makeChangeTravelTimeScale(points)
